...
public class WebServer extends NanoHTTPD {
    ...
    private Response handlePairing(IHTTPSession session) {
        try {
            ...
            Log.i(TAG, "Web API: Received pairing request - port: " + portStr);
    ...
    private Response handleStatus() {
        ...
        try {
            state.put("lastStatus", lastStatus);
            state.put("currentPort", currentPort);
            state.put("isPaired", isPaired);
            state.put("adb5555Available", adbTargetAvailable);
            state.put("targetPort", targetPort);
            state.put("webServerEnabled", webServerEnabled);
            long checked = prefs.getLong("last_guard_check", 0);
            long age = System.currentTimeMillis() - checked;
            state.put("guardHealthy", checked > 0 && age >= 0 && age < 90_000 && prefs.getBoolean("guard_healthy", false));
            state.put("lastGuardCheck", checked);
        } catch (org.json.JSONException e) { Log.e(TAG, "Status encoding failed", e); }
    ...
    private void requestGuardCheck() {
        if (context instanceof AdbConfigService) ((AdbConfigService) context).requestHealthCheck();
        else context.startForegroundService(new android.content.Intent(context, AdbConfigService.class));
    }
