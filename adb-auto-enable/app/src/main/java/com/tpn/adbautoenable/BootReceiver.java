...
public class BootReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Intent.ACTION_BOOT_COMPLETED.equals(action) ||
                Intent.ACTION_LOCKED_BOOT_COMPLETED.equals(action) || Intent.ACTION_MY_PACKAGE_REPLACED.equals(action) || Intent.ACTION_USER_UNLOCKED.equals(action)) {
    ...
    private void startServiceNow(Context context) {
        ...
        RecoveryJob.schedule(context, false);
        try { context.startForegroundService(serviceIntent); }
        catch (RuntimeException e) { Log.e(TAG, "Service start deferred", e); RecoveryJob.schedule(context, true); }
    }
}